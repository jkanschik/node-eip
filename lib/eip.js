...
//				route.sendEventToProcessor(proc, event);
//			});
		}
...
var setExceptionHeader = function(event, error) {
	
};

Route.prototype.dispatchEvent = function(processor, event) {
	if (event.headers._exception) {
		event.headers._exception.processor = processor;
		if (!event.headers._exception.cause)
			event.headers._exception.cause = new Error("Unknown error");
		if (!event.headers._exception.timestamp)
			event.headers._exception.timestamp = new Date();
		if (!event.headers._exception.numberOfAttempts)
			event.headers._exception.numberOfAttempts = 1;
		
		if (event.headers._exception.numberOfAttempts < 3) {
			var self = this;
			setTimeout(function() {
			...
			if (this.options.isErrorRoute) {
				console.log("Unrecoverable error in error route, give up processing event %j.", event);
			} else {
...
Route.prototype.sendEventToProcessorSync = function(processor, event) {
	...
	try {
		numberOfAttempts = event.headers._exception ? event.headers._exception.numberOfAttempts : 0;
		numberOfAttempts += 1;
		...
		this.dispatchEvent(processor, event);
	}
};
...
Processor.prototype.emitEvent = function(event) {
	this.route.dispatchEvent(this, event);
