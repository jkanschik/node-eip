...
Route.prototype.dispatchEvent = function(processor, err, event) {
	...
	if (err) {
		...
		if (event.headers._exception.numberOfAttempts < retryLimit) {
			...
			setTimeout(function() {
				self._sendEventToProcessorSync(procMeta.processor, event);
			}, delay);
		} else {
			if (this.errorRoute) {
		...
		if (procMeta.next)
			this.sendEventToProcessor(procMeta.next, event);
	}
};

Route.prototype.sendEventToProcessor = function(procMeta, event) {
	if (this.sync) {
		this._sendEventToProcessorSync(procMeta.processor, event);
	} else {
		if (procMeta.processor.state === ProcessorStates.SHUTTING_DOWN ||
			procMeta.processor.state === ProcessorStates.SHUT_DOWN) {
			throw new Error("Processor is shutting down, no events are accepted!");
		}
