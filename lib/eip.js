...
Route.prototype.sendEventToProcessor = function(processor, event) {
	if (this.sync) {
		this.sendEventToProcessorSync(processor, event, 0);
	} else {
		var self = this;
		process.nextTick(function() {
			self.sendEventToProcessorSync(processor, event, 0);
		});
	}
	
};

Route.prototype.sendEventToProcessorSync = function(processor, event, numberOfAttempts) {
	var self = this;
	try {
		numberOfAttempts += 1;
		processor.data.apply(processor, [event]);
	} catch (err) {
		if (numberOfAttempts == Route.config.retryLimit) {
			console.log("Error '%s' occured, retries exhausted after %d tries, give up and send to error route.", err, numberOfAttempts);
			this.errorRoute.inject(err);
		} else {
			console.log("Error '%s' occured, retry in %d ms.", err, Route.config.retryDelay);
			setTimeout(function() {
				self.sendEventToProcessorSync(processor, event, numberOfAttempts);
			}, Route.config.retryDelay);
		}
	}
};
